import { List, DataTable, EditButton, DeleteButton, useResourceContext, CreateButton, ShowButton, DateField, ReferenceField, TextField, ReferenceInput, SelectInput } from 'react-admin';
import { Box, Card, CardContent, Typography, Stack, Tooltip } from '@mui/material';
import Grid from '@mui/material/Grid';
import { useCan } from '../../../components/permissions/user-can';
import { ListBreadcrumbs } from '../../../../ListBreadcrumbs';

const LivestockWeightRecordFilters = [
    <ReferenceInput source="livestock_id" reference="livestocks" alwaysOn>
        <SelectInput optionText="tag_no" label="Livestock Tag No" />
    </ReferenceInput>,
];

export const LivestockWeightRecordsList = () => {
    const can = useCan();
    const resource = useResourceContext() ?? "livestock-weight-records";
        ...
        <Box sx={{ p: 2 }}>
            <Grid container spacing={2} alignItems="center" justifyContent="space-between" mb={1}>
                <Grid size={{ xs: 12, md: 6 }}>
                    <Typography variant="h5" fontWeight="bold">Livestock Weight Records</Typography>
                    <Typography variant="body2" color="text.secondary">Track weight changes over time</Typography>
                </Grid>
            ...
            <Card sx={{ borderRadius: 3, boxShadow: 3, overflow: "hidden" }}>
                <CardContent>
                    <List title={false} filters={LivestockWeightRecordFilters} actions={false}>
                        <DataTable rowClick="show" sx={{ '& .RaDataTable-headerCell': { fontWeight: "bold", backgroundColor: "#f5f5f5" } }}>
                            ...
                            <DataTable.NumberCol source="weight" label="Weight (kg)" />
                                ...
                                <Stack direction="row" spacing={1} alignItems="center">
                                    ...
                                    {canDelete && (
                                        <Tooltip title="Delete Record">
                                            <span>
                                                <DeleteButton label={false} mutationMode="pessimistic" confirmTitle="⚠️ Confirm deletion" confirmContent="This will remove the weight record." sx={{ minWidth: 36 }} />
                                            </span>
                                        </Tooltip>
                                    )}
                                </Stack>
